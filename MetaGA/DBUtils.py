...
class DBUtils:
    ...
    def insertPortfolioMapping(self, metaIndividualId, feederIndividualId, firstGeneration, lastGeneration):
        global databaseObject
        query = "INSERT INTO mapping_table" \
                " (meta_individual_id, feeder_individual_id, first_generation, last_generation)" \
                " VALUES" \
                " ( " + str(metaIndividualId) + ", " + str(feederIndividualId) + ", " + str(firstGeneration) + ", " + str(lastGeneration) + " )"
        return databaseObject.Execute(query)
            ...
            for feederIndividualId, generation in resultCurrent:
                if feederIndividualId==oldFeederIndividualId:
                    query = "INSERT INTO mapping_table" \
                            " (meta_individual_id, feeder_individual_id, first_generation, last_generation)" \
                            " VALUES" \
                            " ( " + str(newMetaId+1) + ", " + str(newFeederIndividualId) + ", " + str(generation) + ", " + str(generation) + " )"
                    databaseObject.Execute(query)
                else:
                    query = "INSERT INTO mapping_table" \
                            " (meta_individual_id, feeder_individual_id, first_generation, last_generation)" \
                            " VALUES" \
                            " ( " + str(newMetaId+1) + ", " + str(feederIndividualId) + ", " + str(generation) + ", " + str(generation) + " )"
                    databaseObject.Execute(query)
    ...
    def getPortfolios(self, generation):
        global databaseObject
        query = "SELECT meta_individual_id, COUNT(*) FROM mapping_table WHERE last_generation=" + \
    ...
    def getPortfolioSizeByOffset(self, metaPortfolioIdOffset, generation):
        global databaseObject
        queryId = "SELECT DISTINCT(meta_individual_id), 1 FROM mapping_table WHERE last_generation=" + str(generation) + \
        ...
        if crossoverType==1:
            for i in range(0, cut11, 1):
                query = "INSERT INTO mapping_table" \
                        " (meta_individual_id, feeder_individual_id, first_generation, last_generation)" \
                        " VALUES" \
                        " ( " + str(newId+1) + ", " + str(list1[i]) + ", " + str(generation) + ", " + str(generation) + " )"
                databaseObject.Execute(query)
            for i in range(cut21, len(list2), 1):
                query = "INSERT INTO mapping_table" \
                        " (meta_individual_id, feeder_individual_id, first_generation, last_generation)" \
                        " VALUES" \
                        " ( " + str(newId+1) + ", " + str(list2[i]) + ", " + str(generation) + ", " + str(generation) + " )"
                databaseObject.Execute(query)
            ...
            if numChildren==2:
                for i in range(0, cut21, 1):
                    query = "INSERT INTO mapping_table" \
                            " (meta_individual_id, feeder_individual_id, first_generation, last_generation)" \
                            " VALUES" \
                            " ( " + str(newId+2) + ", " + str(list2[i]) + ", " + str(generation) + ", " + str(generation) + " )"
                    databaseObject.Execute(query)
                for i in range(cut11, len(list1), 1):
                    query = "INSERT INTO mapping_table" \
                            " (meta_individual_id, feeder_individual_id, first_generation, last_generation)" \
                            " VALUES" \
                            " ( " + str(newId+2) + ", " + str(list1[i]) + ", " + str(generation) + ", " + str(generation) + " )"
                    databaseObject.Execute(query)
    ...
    def updateSelectedPortfolio(self, portfolioId):
        global databaseObject
        queryUpdate = "UPDATE mapping_table SET last_generation=last_generation+1 WHERE meta_individual_id=" + str(portfolioId)
